...
func (s *storage) FindLinkBannerAndSlot(ctx context.Context, bannerGUID, slotGUID *uuid.UUID) (*models.LinkBannerAndSlot, error) {
	query := `
		SELECT guid, banner_guid, slot_guid
		FROM banners_link_slots
		WHERE banner_guid = $1 AND slot_guid = $2
	`
...
func (s *storage) FindStatByParams(ctx context.Context, bannerGUID, slotGUID, socialGroupGUID *uuid.UUID) (*models.Stat, error) {
	query := `
		SELECT guid, banner_guid, slot_guid, social_group_guid, shows, clicks
		FROM stats
		WHERE banner_guid = $1 AND slot_guid = $2 AND social_group_guid = $3
	`
...
func (s *storage) CreateStat(ctx context.Context, val *models.Stat) error {
	sql := `
		INSERT INTO stats(guid, banner_guid, slot_guid, social_group_guid, shows, clicks)
		VALUES ($1, $2, $3, $4, $5, $6);
	`
...
func (s *storage) FindStatsBySlotAndSocialGroup(ctx context.Context, slotGUID, statGUID *uuid.UUID) ([]*models.Stat, error) {
	sql := `
		SELECT guid, banner_guid, slot_guid, social_group_guid, shows, clicks
		FROM stats
